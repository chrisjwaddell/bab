...
const fileExcludeSpecific = (file) =>
	excludeSpecificFilepaths.find((cv) => cv === file)

const textLine = (txt) => txt.replace(/\r?\n?/g, "").trim()

const strtopCol1 = (filename) => topCol1.replace("{F}", filename)
const strresultCol1 = (filename, linenumber) =>
	resultsCol1.replace("{F}", filename).replace("{N}", linenumber)
			...
			lines.forEach((cv, i) => {
				...
				if (foundLine !== -1) {
					if (i - foundLine >= 0 && i - foundLine <= linesAfter) {
						strOutput +=
							strresultCol1(filePath, String(linenumber + 1)) +
							fieldSeperator +
							textLine(cv) +
							fieldSeperator +
							"\r\n"
						...
						strOutput +=
							strbottomCol1(filePath) +
							fieldSeperator +
							strbottomCol2(filePath) +
							fieldSeperator +
							strbottomCol3(filePath) +
							"\r\n"
					}
				}
			})
...
async function argsRead() {
	return new Promise(async (resolve, reject) => {
		...
		if (args.findIndex((cv) => cv === "-s" || cv === "--settings") !== -1) {
			if (
				typeof args[
					args.findIndex((cv) => cv === "-s" || cv === "--settings") +
						1
				] !== "undefined"
			) {
				let settingsargtfile =
					args[
						args.findIndex(
							(cv) => cv === "-s" || cv === "--settings"
						) + 1
				...
				try {
					if (fs.existsSync(path.join(__dirname, settingsargtfile))) {
						//file exists
						await configFileRead(
							path.join(__dirname, settingsargtfile)
						)
					} else {
						if (
							fs.existsSync(path.join(appRoot, settingsargtfile))
						) {
							await configFileRead(
								path.join(appRoot, settingsargtfile)
							)
						} else {
							console.log(
								"Settings file - " +
									settingsargtfile +
									" doesn't exist. Resuming search anyway."
							)
						}
					}
				} catch (err) {
					console.log(
						"There was a problem reading settings file - " +
							settingsargtfile +
							". Resuming search anyway."
					)
				}
			} else {
				console.log(
					"Settings file is missing. The search is resuming without it."
				)
			}
		}
		...
		if (typeof args[1] === "undefined") {
			...
			if (arrFiletype === "") {
				console.log(
					"Error in filetype expression. Use square brackets [ ] for more than one file type. Can't search on text."
				)
				searchSummary +=
		...
		if (args.findIndex((cv) => cv === "-o") !== -1) {
			if (
				typeof args[args.findIndex((cv) => cv === "-o") + 1] !==
				"undefined"
			) {
				outputfile = args[args.findIndex((cv) => cv === "-o") + 1]
			} else {
				console.log(
					"-o switch in but no output file. Output will go to the screen."
				)
			}
		}
	...
	function regularExpressionWrong() {
		strError =
			"Error - Invalid Regular expression for the search string." + "\r\n"
		strError +=
			"You can use '!' in the search string to enter special characters such as *, ' in the regular expression." +
			"\r\n"
		strError +=
			"USAGE: bab [search-string] [filetype] [options] \r\nTry 'bab --help' for more information."
	...
	function helpInfo() {
		strHelpInfo = `USAGE: bab [search-string] [filetype] [options]
...
-o [output-file]        Search results go into this output file. This can be relative (relative to the
                        ...
                        'bash -c 'bab "search string" "txt" > output.txt'.
							...
							let p = new Promise(async (resolve) => {
								...
								if (li === -1) {
									...
									searchSummary +=
										"\r\nFiletype - " +
										arrFiletype[i] +
										" has no dot (.), each filetype must be in this format: <filename>.<extension> eg *.txt or *logs.htm?"
								} else {
									// console.log(arrFiletype[i])
									await directoriesLoop(
										dirRoot,
										arrFiletype[i]
									)
										.then((d) => {
	...
	return new Promise(async (resolve, reject) => {
		...
		for (let i = 0; i < l; i++) {
			...
			if (files[i][0] !== "$") {
				if (
					!dirExclude(files[i]) &&
					!dirExcludeSpecific(fileName) &&
					!fileExclude(files[i]) &&
					!fileExcludeSpecific(fileName)
				) {
					try {
						...
						// if (e.code !== "EPERM") {
						console.log(
							"An error has occurred. If this is a file or directory you don't have the right permissions for, you may need to add it to the exclude directories or files settings. See the ./settings.js file."
						)
						console.error(e)
						// }
					}
...
function filenameValid(filename, filetype) {
	...
	let replaceQuestion =
		replaceStars.replace(/\?/g, strCharsAllowedWindows + "?") + "$"
